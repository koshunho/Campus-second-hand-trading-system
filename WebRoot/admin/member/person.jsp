...
</table><form action="<%=basePath %>MemberManage.do?method=DELMEMBER" method="post" name="pageform">
	...
	function allch()
	{
		for(i=0;i<<%=pagelist3.size()%>;i++)
		{
		  ...
		  <TR align="center" bgColor=#ffffff>
			...
			<TD id=map><%=tb.getNum(pagelist2.get(1).toString()) %></TD>
			<TD id=map><%=tb.getNum(pagelist2.get(1).toString())>500?"VIP会员":"普通会员" %></TD>
			...
			<TD id=map><a href="<%=basePath%>ComServlet?method=delm&id=<%=pagelist2.get(0).toString()%>" >删除</a></TD>
		  </TR>
		<%}%>    	
