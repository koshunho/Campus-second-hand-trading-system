...
<script  language="javascript" >
function top2(){
   	form3.action="<%=basePath%>jg.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%>jg.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>jg.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>jg.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>jg.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>jg.jsp?page="+page;
      form3.submit();
...
//-->
     ...
     <TABLE class=dragTable cellSpacing=0 cellPadding=0 width="100%" border=0>
		...
		<TD  class=middle align="left" >
			...
			List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from sp  where jz='未'  order by jg asc",8);
