     ...
     <TABLE class=dragTable cellSpacing=0 cellPadding=0 width="100%" border=0>
		...
		<TD  class=middle align="left" >
               <TABLE class=xsnr id=tb1_nr1  cellSpacing=0 cellPadding=0 width="100%" border=0>
                    ...
                    List hotlist=cb.get1Com("select * from sp where id='"+id+"'",8);
                    if(!hotlist.isEmpty()){
         			%>
        <TR>
		  ...
		  <TD align=left><br>
		  商品名称：<%=hotlist.get(1).toString()%>  &nbsp;&nbsp;&nbsp;商品分类：<%=hotlist.get(2).toString()%><br>
		  商品价格：<%=hotlist.get(4).toString()%>   <br>
		  商品介绍：<%=hotlist.get(6).toString()%>    <br>  
		  <a href="<%=basePath%>prepinfo.jsp?spid=<%=id%>&title=<%=hotlist.get(1).toString()%>">加入购物车</a> （如果您还没有登录，请先登录再进行预定操作！）
		  </TD>
		</TR>
